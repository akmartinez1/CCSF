...
GitHub:  https://github.com/akmartinez1/CCSF-CS110B--VS-/tree/master/amartinezHW5
...
int main(void)
{
	const int size = 5;
	string teams[size] = { "", "", "", "", "" };
	int wins[size] = { 0, 0, 0, 0, 0 };
	char run_again = 'y';
		
		initializeData(teams, wins, size);
...
{
	cout << "\n\nThis program will take input of five baseball teams and their wins\nand sort them by rank of best to worse.\n\n";
	...
	for (int input_loop = 0; input_loop < idSize; input_loop++)
	{
		...
		/*
		Initially experienced issue with getline where after the first iteration of input loop the getline statement captured white space
		information left in buffer and loop bypassed taking input from user.

		Learned from internet research that cin.ignore(); might correct behavior.
		http://stackoverflow.com/questions/10553597/cin-and-getline-skipping-input/10553849#10553849
		*/
...
void displayData(string ddTeams[], int ddWins[], int ddSize)
{
	...
	for (int output_loop = 0; output_loop < 5; output_loop++)
	{
